...
さらに、テストに使ったのは「バイトモード + Shift_JIS バイト列」の QR だが、
QR の規格には Shift_JIS 前提の **Kanji モード**があり、日本製の生成ツールは
そちらを使う可能性が高い。Kanji モードの QR がカメラ経路で読めるかは**未確認**。
これが読めるなら、カメラ経路の Shift_JIS 問題はさらに稀なケースということになる。
...
- **カメラ経路で Shift_JIS の QR が読めない**（優先度: 要判断）
  `GmsBarcodeScanner` では認識されず、バンドル版 ML Kit では読める（サイクル4で実測）。
  直すならカメラ経路を `GmsBarcodeScanner` から CameraX + バンドル版 ML Kit に置き換える。
  **依存関係の追加は不要**（シリアル OCR 機能のために両方とも既に入っている）。
  ただしスキャナー画面を自前実装することになり、変更は大きい。

  **着手前に判断すること**: そもそも実利用で Shift_JIS の QR にどれだけ遭遇するのか。
  サイクル4で「珍しくない」という前提が未検証だったと判明している。
  まず Kanji モードの QR（`06-kanji-mode.png`）がカメラ経路で読めるかを確認し、
  読めるなら影響範囲はさらに狭い。割に合わないなら着手しない判断も正しい。
